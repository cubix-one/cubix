...
export default async function ServeAction(projectPath: string, props: { port: number; address: string }) {
  p.intro(`${color.inverse('🚀 Starting Rojo Server...')}`);
  const { ip, port: finalPort } = await RojoServer(projectPath, props);
